...
import { getIndicators, habitDates, weeklyProgress } from '../lib/streakflow'
...
export default function ProgressPanel({ habits, records, settings, today }: { habits: Habit[]; records: CheckIn[]; settings: Settings; today: string }) {
  ...
  return <>
    ...
    })}</div><p className="muted mt-4">Parciais e ignorados não contam como conclusões. Dias sem hábitos aparecem com 0%.</p></section>}
    <section className="panel"><div className="section-heading"><div><h2>Consistência por hábito</h2><p className="muted">Semana iniciada {settings.firstDayOfWeek === 0 ? 'no domingo' : 'na segunda-feira'}.</p></div></div>{!habits.length ? <EmptyState title="Sua rotina começa aqui." text="Crie um hábito para acompanhar sua meta semanal." createHabit /> : habits.map(habit => {
      ...
      return <article className="consistency-row" key={habit.id}><div><h3>{habit.title}</h3><p className="muted">Meta: {habit.weeklyGoal}x / semana · Realizado: {week.count}x</p><span className="streak-inline"><Mascot size={20} />{streak(habitDates(records, habit.id), now)} dias seguidos</span></div><div className="habit-weekly"><strong>{week.percent}%</strong><progress aria-label={`Meta semanal de ${habit.title}`} max={100} value={week.percent} /></div></article>
    })}</section>
  </>
}
